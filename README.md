<div align=center><h1>Terrafrom</h1></div>
...
Terraform tutorials online often demonstrate a directory structure consisting of three files: 
     ...
     1. main.tf: contains all providers, resources and data sources (Main configuration file - where we add the resources (cloud service and its configs), and data sources)
     2. variables.tf: contains all defined variables (We will add the imp variables into this file).
        2.1 .tfvars (we will use this file to pass the variables)
     3.  output.tf: contains all output resources
      
<b>The issue</b> with this structure is that most logic is stored in the single main.tf file which therefore becomes pretty complex and long.

<i>Terraform</i> however, does not mandate this structure, it only requires a directory of Terraform files.
Since the filenames do not matter to Terraform I propose to use a structure that enables users to quickly understand the code.

<b>preferable Structure:</b>

        provider.tf: contains the terraform block and provider block (Here, we will define the cloud provider[AWS, GCP, AZURE], region)
...
<div align = center><h1>Important Terraform Commands</h1></div>
...
### Terraform Workspaces
                    1. '''terraform workspace new'''	Create a new workspace and select it
                    2. '''terraform workspace select'''	Select an existing workspace
                    3. '''terraform workspace list'''	List the existing workspaces
                    4. '''terraform workspace show'''	Show the name of the current workspace
                    5. '''terraform workspace delete'''	Delete an empty workspace
...
<b><i>This state file is extremely important;</i></b> it maps various resource metadata to actual resource IDs so that Terraform knows what it is managing.
...
We can simply Copy and paste this configuration in our source code editor in backend.tf file.
...
#### before that we have to create resource are s3 and DynamoDB that resource will call in backend.tf

###### Creating our DynamoDB Table 
- Create a new file in your working directory labeled dynamo.tf

Copy and paste this configuration in our source code editor in your main.tf file.
```
# S3 
...
- It provides dynamic information about the entities we define outside of terraform.
- Data Sources allow fetching data about the infrastructure components’ configuration.
- It allows to fetch data from the cloud provider APIs using terraform scripts.
- When we refer to a resource using a data source, it won’t create the resource.
- Instead, they get information about that resource so that we can use it in further configuration if required.

2. How to use Data Source?
